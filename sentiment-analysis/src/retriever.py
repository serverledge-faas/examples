...
def upload_to_minio(client, input, object_name):
    print(f" [Retrieval] Uploading file to MinIO {object_name}...")
    if client.exists(object_name=object_name):
        print(f" [Retrieval] Upload not performed, object {object_name} already exists")
        return False
    else:
        client.upload_file(input, object_name, override=False)
    return True

def handler(minio_endpoint, minio_access_key, minio_secret_key, data_url, local_temp_path, object_name):
    client = minio_client.MinIoClient(minio_endpoint, minio_access_key, minio_secret_key)
    ...
    uploaded = upload_to_minio(client, local_temp_path, object_name=object_name)
